# Kimi CLI — full-featured with all tools pre-installed
...
RUN python3 -c "exec(open('/tmp/system_prompt.py').read()); open('/root/.mm-claude/CLAUDE.md','w').write(CLAUDE_SYSTEM_PROMPT)" \
    && python3 -c "exec(open('/tmp/system_prompt.py').read()); open('/root/.kimi-system-prompt.md','w').write(SYSTEM_PROMPT)" \
